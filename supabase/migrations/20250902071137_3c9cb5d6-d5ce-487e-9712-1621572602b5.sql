...
-- Create function to check and update leave quota
CREATE OR REPLACE FUNCTION public.check_leave_quota()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if this is a sick leave and if student has exceeded monthly limit
  IF NEW.leave_type = 'sick' THEN
    -- Get current sick leaves for this month
    WITH current_quota AS (
      SELECT sick_leaves_used 
      FROM public.student_leave_quota 
      WHERE student_id = NEW.student_id 
        AND month = EXTRACT(MONTH FROM NEW.start_date)
        AND year = EXTRACT(YEAR FROM NEW.start_date)
    )
    SELECT COALESCE(sick_leaves_used, 0) + 1 INTO leave_count
    FROM current_quota;
    
    -- If more than 2 sick leaves in a month or leave is more than 3 days, require document
    IF leave_count > 2 OR (NEW.end_date - NEW.start_date + 1) > 3 THEN
      IF NEW.document_url IS NULL THEN
        RAISE EXCEPTION 'Document required for sick leave exceeding 2 per month or more than 3 days';
      END IF;
    END IF;
  END IF;
